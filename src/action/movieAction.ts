...
export const getMovie = (page: number, title: string) => async (
  ...
  try {
    ...
    const res = await axios.get(
      `https://omdbapi.com/?apikey=2edf5c13&s=${title}&page=${page}&type=movie`
    );
    dispatch({
      type: MOVIE_SUCCESS,
...
export const getMovieDetails = (id: string) => async (
  ...
  try {
    ...
    const res = await axios.get(
      `https://omdbapi.com/?apikey=2edf5c13&i=${id}&plot=full`
    );
    dispatch({
